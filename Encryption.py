...
fromos.path import isfile, join
import time
classEncryptor:
...
defencrypt_file(self, file_name):
...
defdecrypt_file(self, file_name):
with open(file_name, 'rb') as fo:
ciphertext = fo.read()
...
key=
b'[EX\xc8\xd5\xbfI{\xa2$\x05(\xd5\x18\xbf\xc0\x85)\x10nc\x94\x02)j\xdf\xcb\xc4\
x94\x9d(\x9e'
enc = Encryptor(key)
